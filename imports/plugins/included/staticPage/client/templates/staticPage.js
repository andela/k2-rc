...
Template.staticPage.onRendered(() => {
  tinymce.init({
    ...
    toolbar1:
      "formatselect | bold italic strikethrough fontselect fontsizeselect forecolor backcolor | link | alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent  | removeformat"
  });
});
...
Template.staticPagePanel.events({
  ...
  "click .editPage": function (event) {
    ...
    $(".static-page")
      .find("#sp-show")
      .prop("checked", pageDetails[0].isEnabled === true);
    $(".static-page")
      .find(".edit-page-data")
...
Template.staticPageForm.events({
  "submit form": event => {
    ...
    if (
      ...
      Meteor.call("insertPage", pageName, pageAddress, pageContent, userId, shopId, isEnabled, createdAt, err => {
        if (err) {
      ...
      if (pageDetails.length > 0) {
        ...
        Meteor.call("updatePage", _id, pageName, pageAddress, pageContent, userId, shopId, isEnabled, createdAt, updatedAt, err => {
          if (err) {
            Alerts.toast(err.message, "error");
          } else {
